 ...
 * FileName: screenshotComparatorUtils.ts
...
async function captureAndCompareScreenshot(page: Page, testName: string, browserName: string): Promise<void> {
    ...
    const diff = await new Promise<resemble.ComparisonResult>((resolve) => {
        resemble(readFileSync(originalImagePath))
            .compareTo(readFileSync(newImagePath))
            .onComplete(resolve);
    });

    if (!diff.getBuffer) {
        throw new Error("❌ Diff buffer method is undefined!");
    }
    const diffBuffer = diff.getBuffer(true);
    if (!diffBuffer) {
        throw new Error("❌ Diff buffer is undefined!");
    }
